...
class list
{
  ...
  public:
    ...
    void reorder(int pos,std::vector<int> selected) {
        ...
        if (mSize == 0 || selected.size() == 0) return;

        auto it = begin();
        auto ins_it = end();

        CP::list<T> l;
        auto v_it = selected.begin();
        int n = mSize;
        for (int i = 0; i < n; i++) {
        ...
        for (auto &x : l) {
            // std::cout << "ins_it = " << *ins_it << std::endl;
            // std::cout << "x: " << x << std::endl;
            insert(ins_it, x);
        }
        // ins_ptr->prev->next = l.mHeader->next;
        // l.mHeader->next->prev = ins_ptr->prev;
        // l.mHeader->prev->next = ins_ptr;
        // ins_ptr->prev = l.mHeader->prev;
    }
